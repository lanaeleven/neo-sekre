...
@section('container')
    <div>
        <x-title-with-back-button title="surat masuk - sudah diteruskan" backUrl="/" />
        
        <div>
            <form class="row g-1" action="/surat-masuk/ns/sudah-diteruskan">
                    ...
                    <x-input-field-filter :isLabel="false" name="status" type="text" placeholder="status" :isMedium="true" />
                    <x-filter-submit-button />
            </form>
        </div>
        
        @if ($suratMasuk->isEmpty())
            <x-empty-data data='Surat Masuk yang Sudah Diteruskan' />
        @else
