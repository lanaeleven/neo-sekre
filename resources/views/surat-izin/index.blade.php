...
@section('container')
    ...
    <x-default-page-container>
        ...
        {{-- NAVBAR --}}
        <x-navbar title="{{ $title }}" urlTambah="/surat-izin/tambah" />
        ...
        {{-- ================= DESKTOP FILTER BAR ================= --}}
        <x-desktop-filter-bar urlFilter="/surat-izin/index">
            <x-range-date-filter />
                ...
                <tr class="hover:bg-green-50 transition-colors">
                    ...
                    <x-td-action>
                        <x-button-with-tooltip variant="warning" tooltip="Edit"
                            url="/surat-izin/edit/{{ $si->id }}"><x-heroicon-s-pencil
                                class="w-4 h-4" /></x-button-with-tooltip>
                        <x-button-with-tooltip variant="light" tooltip="Lihat" url="{{ asset('storage/' . $si->filePath) }}"
                        ...
                        <x-button-with-tooltip variant="success" tooltip="Disposisi"
                            url="/surat-izin/disposisi/{{ $si->id }}"><x-heroicon-s-arrow-up-right
                                class="w-4 h-4" /></x-button-with-tooltip>
                    </x-td-action>
                </tr>
            @endforeach
            ...
            @foreach ($suratIzin as $si)
                <x-mobile-card-border>
                    ...
                    <x-mobile-card-text-actions>
                        <x-button-with-tooltip variant="warning" tooltip="Edit"
                            url="/surat-izin/edit/{{ $si->id }}">Edit</x-button-with-tooltip>
                        <x-button-with-tooltip variant="light" tooltip="Lihat"
                            url="{{ asset('storage/' . $si->filePath) }}" target="_blank">Lihat</x-button-with-tooltip>
                        <x-button-with-tooltip variant="info" tooltip="Lacak"
                            url="/surat-izin/lacak-distribusi/{{ $si->id }}">Lacak</x-button-with-tooltip>
                        <x-button-with-tooltip variant="success" tooltip="Disposisi"
                            url="/surat-izin/disposisi/{{ $si->id }}">Disposisi</x-button-with-tooltip>
                    </x-mobile-card-text-actions>
                </x-mobile-card-border>
            @endforeach
        </x-mobile-card-container>
